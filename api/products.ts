// /api/products.ts
export type Product = {
  id: string;          // productId interno (usado pelo seu front)
  externalId: string;  // externalId enviado ao AbacatePay (e retornado no webhook)
  title: string;
  price_cents: number;
  file: string;        // URL pública (Google Drive) ou link direto
};

// Catalogo de produtos (use estes externalId na criação da cobrança)
export const PRODUCTS: Record<string, Product> = {
  "product-001": {
    id: "product-001",
    externalId: "prod_robot_ai",
  ...
  "product-003": {
    id: "product-003",
    externalId: "prod_catalogo_exclusivo",
...
// Helper: map externalId -> product (usado no webhook)
export const PRODUCTS_BY_EXTERNAL: Record<string, Product> = Object.values(PRODUCTS).reduce((acc, p) => {
  acc[p.externalId] = p;
  return acc;
